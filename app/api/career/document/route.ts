...
import { draftDocument, anthropicConfigured, type DocKind } from "@/lib/agents";

export const dynamic = "force-dynamic";
export const maxDuration = 60;

// Cover letter / сообщение рекрутеру. Текст возвращается для ручной отправки —
// ничего не отправляется автоматически. Требует профиль с cv_text.
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const docKind: DocKind = kind === "recruiter" ? "recruiter" : "cover";
    ...
    const { text } = await draftDocument(docKind, vac, profile);
